...
module.exports.run = async function ({
    ...
    try {
        ...
        case "-s": {
            ...
            global.baucua.set(threadID, gameThread);
            return api.sendMessage(`Bắt đầu! Xin mời ${gameThread.player.length} người chơi chọn linh vật muốn chọn và số tiền muốn cược\nVí dụ: tôm 293`, threadID);
            break;
        }
        case "end":
        case "-e": {
        ...
        default: {
            return api.sendMessage("Hướng dẫn sử dụng bầu cua\n\n|baucua create: để tạo bàn chơi\n|baucua start : để bắt đầu ván đấu\n|baucua join : để tham gia ván đấu\n|baucua leave : để rời khỏi bàn chơi \n|balance : để xem số dư tài khoản", threadID, messageID);
        }
        }
    } catch (err) {
